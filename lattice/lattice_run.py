...
def encrypt(message: str, n: int, noise_level: float) -> Tuple[List[List[float]], int]:
    # Encrypts a message using the GGH cryptosystem
...
def main():
    ...
    while True:
        try:
            runtime_input = input("Please enter the runtime limit for the script in hours: ")
    ...
    print("=" * 60)
    print("Lattice-based Cryptography Scaling Analysis - EXPERIMENT 1: RUNTIME")
    print(f"Message: '{message}'")
    print(f"Noise Level: {noise_level}")
    print(f"Test Dimensions: {min_dim} to {max_dim} ({TRIALS_PER_DIMENSION} trials per dimension)")
    print(f"TIME LIMIT: {RUNTIME_LIMIT_HOURS} HOUR(S)")
    print("=" * 60)
    ...
    for n in range(min_dim, max_dim + 1):
        ...
        if elapsed_seconds > RUNTIME_LIMIT_HOURS * 60 * 60:
            print(f"\nTime limit of {RUNTIME_LIMIT_HOURS} hour(s) reached. Finalizing results.")
            break
...
def plot_results(csv_path):
    # Plots the decryption times
    print(f"\nGenerating plots from {csv_path}...")
    try:
        df = pd.read_csv(csv_path)
        
        plt.figure(figsize=(12, 7))
        plt.plot(df['Dimension'], df['avg_Decrypt_Time_Good'] * 1000, label='Decryption with good basis (Private Key)', color='green')
        plt.plot(df['Dimension'], df['avg_Decrypt_Time_Bad'] * 1000, label='Decryption with bad basis (Public Key)', color='red')
        
        plt.title('Decryption Time vs. Lattice Dimension (Linear Scale)', fontsize=16)
        ...
        plt.title('Decryption Time vs. Lattice Dimension (Logarithmic Scale)', fontsize=16)
        plt.xlabel('Lattice Dimension (n)', fontsize=12)
        plt.ylabel('Time (Milliseconds)', fontsize=12)
        plt.legend()
        plt.grid(True, which='both', linestyle='--', linewidth=0.5)
